...
func (r *Registry) canI(raw json.RawMessage) (mcp.ToolResult, error) {
	args, err := decode[canIArgs](raw); if err != nil { return mcp.ToolResult{}, err }
	allowed, reason, err := r.Authz.CanI(r.ctx, args.Namespace, args.Group, args.Resource, args.Verb, args.Name)
	if err != nil { return mcp.ToolResult{}, err }
...
func (r *Registry) listNamespaces() (mcp.ToolResult, error) {
	if err := r.authorize("", "", "namespaces", "list", ""); err != nil { return mcp.ToolResult{}, err }
...
func (r *Registry) listNodes() (mcp.ToolResult, error) {
	if err := r.authorize("", "", "nodes", "list", ""); err != nil { return mcp.ToolResult{}, err }
	list, err := r.Kube.Clientset.CoreV1().Nodes().List(r.ctx, metav1.ListOptions{})
	if err != nil { return mcp.ToolResult{}, err }
	items := make([]map[string]any, 0, len(list.Items))
	for _, n := range list.Items {
		ready := "Unknown"
		for _, c := range n.Status.Conditions { if c.Type == corev1.NodeReady { ready = string(c.Status) } }
...
func (r *Registry) describeNode(raw json.RawMessage) (mcp.ToolResult, error) {
	args, err := decode[nameArgs](raw); if err != nil { return mcp.ToolResult{}, err }
	if args.Name == "" { return mcp.ToolResult{}, fmt.Errorf("name is required") }
	if err := r.authorize("", "", "nodes", "get", args.Name); err != nil { return mcp.ToolResult{}, err }
...
func (r *Registry) listPods(raw json.RawMessage) (mcp.ToolResult, error) {
	args, err := decode[listPodsArgs](raw); if err != nil { return mcp.ToolResult{}, err }
	ns := r.Kube.NamespaceOrDefault(args.Namespace)
	if err := r.authorize(authNamespace(ns), "", "pods", "list", ""); err != nil { return mcp.ToolResult{}, err }
...
func (r *Registry) describePod(raw json.RawMessage) (mcp.ToolResult, error) {
	...
	if args.Name == "" { return mcp.ToolResult{}, fmt.Errorf("name is required") }
	ns := r.Kube.NamespaceOrDefault(args.Namespace)
	if err := r.authorize(ns, "", "pods", "get", args.Name); err != nil { return mcp.ToolResult{}, err }
	...
	if r.authorize(ns, "", "events", "list", "") == nil {
		...
		for _, e := range events.Items { if e.Type == corev1.EventTypeWarning { warnings = append(warnings, map[string]any{"reason": e.Reason, "message": e.Message, "count": e.Count, "lastTimestamp": e.LastTimestamp.String()}) } }
	}
	data := map[string]any{"namespace": p.Namespace, "name": p.Name, "phase": string(p.Status.Phase), "ready": podReady(*p), "restarts": podRestarts(*p), "reason": podReason(*p), "node": p.Spec.NodeName, "podIP": p.Status.PodIP, "conditions": p.Status.Conditions, "containers": p.Status.ContainerStatuses, "warningEvents": warnings}
	return ok(fmt.Sprintf("Pod %s/%s is %s, ready %s, restarts %d, reason %q.", p.Namespace, p.Name, p.Status.Phase, podReady(*p), podRestarts(*p), podReason(*p)), data), nil
}

type logsArgs struct { Namespace, Name, Container string; TailLines, SinceSeconds int64 }
func (r *Registry) getPodLogs(raw json.RawMessage) (mcp.ToolResult, error) {
	args, err := decode[logsArgs](raw); if err != nil { return mcp.ToolResult{}, err }
	if args.Name == "" { return mcp.ToolResult{}, fmt.Errorf("name is required") }
	ns := r.Kube.NamespaceOrDefault(args.Namespace)
	if err := r.authorize(ns, "", "pods/log", "get", args.Name); err != nil { return mcp.ToolResult{}, err }
	if args.TailLines == 0 { args.TailLines = 200 }
	opts := &corev1.PodLogOptions{Container: args.Container, TailLines: &args.TailLines}
	if args.SinceSeconds > 0 { opts.SinceSeconds = &args.SinceSeconds }
...
type eventsArgs struct { Namespace, FieldSelector string; Limit int }
func (r *Registry) listEvents(raw json.RawMessage) (mcp.ToolResult, error) {
	args, err := decode[eventsArgs](raw); if err != nil { return mcp.ToolResult{}, err }
	ns := r.Kube.NamespaceOrDefault(args.Namespace)
	if err := r.authorize(authNamespace(ns), "", "events", "list", ""); err != nil { return mcp.ToolResult{}, err }
	list, err := r.Kube.Clientset.CoreV1().Events(ns).List(r.ctx, metav1.ListOptions{FieldSelector: args.FieldSelector})
	if err != nil { return mcp.ToolResult{}, err }
	sort.Slice(list.Items, func(i, j int) bool { return list.Items[i].CreationTimestamp.After(list.Items[j].CreationTimestamp.Time) })
	limit := args.Limit; if limit <= 0 || limit > 100 { limit = 100 }
	items := []map[string]any{}
	for i, e := range list.Items { if i >= limit { break }; items = append(items, map[string]any{"namespace": e.Namespace, "type": e.Type, "reason": e.Reason, "message": e.Message, "count": e.Count, "object": e.InvolvedObject.Kind + "/" + e.InvolvedObject.Name, "age": age(e.CreationTimestamp)}) }
	return ok(fmt.Sprintf("Found %d events, returning %d.", len(list.Items), len(items)), map[string]any{"events": items}), nil
}

type deployArgs struct { Namespace, LabelSelector string; Name string }
func (r *Registry) listDeployments(raw json.RawMessage) (mcp.ToolResult, error) {
	args, err := decode[deployArgs](raw); if err != nil { return mcp.ToolResult{}, err }
	ns := r.Kube.NamespaceOrDefault(args.Namespace)
	if err := r.authorize(authNamespace(ns), "apps", "deployments", "list", ""); err != nil { return mcp.ToolResult{}, err }
	list, err := r.Kube.Clientset.AppsV1().Deployments(ns).List(r.ctx, metav1.ListOptions{LabelSelector: args.LabelSelector})
	if err != nil { return mcp.ToolResult{}, err }
	items := make([]map[string]any, 0, len(list.Items))
	for _, d := range list.Items { replicas := int32(0); if d.Spec.Replicas != nil { replicas = *d.Spec.Replicas }; items = append(items, map[string]any{"namespace": d.Namespace, "name": d.Name, "desired": replicas, "ready": d.Status.ReadyReplicas, "available": d.Status.AvailableReplicas, "updated": d.Status.UpdatedReplicas, "age": age(d.CreationTimestamp)}) }
...
func (r *Registry) describeDeployment(raw json.RawMessage) (mcp.ToolResult, error) {
	...
	if args.Name == "" { return mcp.ToolResult{}, fmt.Errorf("name is required") }
	ns := r.Kube.NamespaceOrDefault(args.Namespace)
	if err := r.authorize(ns, "apps", "deployments", "get", args.Name); err != nil { return mcp.ToolResult{}, err }
	d, err := r.Kube.Clientset.AppsV1().Deployments(ns).Get(r.ctx, args.Name, metav1.GetOptions{})
	if err != nil { return mcp.ToolResult{}, err }
	replicas := int32(0); if d.Spec.Replicas != nil { replicas = *d.Spec.Replicas }
...
func (r *Registry) getResourceUsage(raw json.RawMessage) (mcp.ToolResult, error) {
	args, err := decode[usageArgs](raw); if err != nil { return mcp.ToolResult{}, err }
	if args.Scope == "" { args.Scope = "pods" }
	switch args.Scope {
	case "pods":
		ns := r.Kube.NamespaceOrDefault(args.Namespace)
		if err := r.authorize(authNamespace(ns), "metrics.k8s.io", "pods", "list", ""); err != nil { return mcp.ToolResult{}, err }
	...
	case "nodes":
		if err := r.authorize("", "metrics.k8s.io", "nodes", "list", ""); err != nil { return mcp.ToolResult{}, err }
...
func (r *Registry) findUnhealthy(raw json.RawMessage) (mcp.ToolResult, error) {
	...
	for _, p := range pods.Items { if !isPodHealthy(p) { badPods = append(badPods, map[string]any{"namespace": p.Namespace, "name": p.Name, "phase": string(p.Status.Phase), "ready": podReady(p), "restarts": podRestarts(p), "reason": podReason(p), "node": p.Spec.NodeName}) } }
	...
	if r.authorize(authNamespace(ns), "apps", "deployments", "list", "") == nil {
		...
		for _, d := range deploys.Items { replicas := int32(0); if d.Spec.Replicas != nil { replicas = *d.Spec.Replicas }; if d.Status.ReadyReplicas < replicas || d.Status.AvailableReplicas < replicas { badDeployments = append(badDeployments, map[string]any{"namespace": d.Namespace, "name": d.Name, "desired": replicas, "ready": d.Status.ReadyReplicas, "available": d.Status.AvailableReplicas, "updated": d.Status.UpdatedReplicas}) } }
	}
	warnings := []map[string]any{}
	if r.authorize(authNamespace(ns), "", "events", "list", "") == nil {
		events, _ := r.Kube.Clientset.CoreV1().Events(ns).List(r.ctx, metav1.ListOptions{})
		for _, e := range events.Items { if e.Type == corev1.EventTypeWarning { warnings = append(warnings, map[string]any{"namespace": e.Namespace, "reason": e.Reason, "message": e.Message, "object": e.InvolvedObject.Kind + "/" + e.InvolvedObject.Name, "count": e.Count}) } }
	}
...
func (r *Registry) explainResource(raw json.RawMessage) (mcp.ToolResult, error) {
	args, err := decode[explainArgs](raw); if err != nil { return mcp.ToolResult{}, err }
	if args.APIVersion == "" || args.Kind == "" || args.Name == "" { return mcp.ToolResult{}, fmt.Errorf("apiVersion, kind, and name are required") }
	...
	ns := r.Kube.NamespaceOrDefault(args.Namespace)
	if mapping.Scope.Name() == meta.RESTScopeNameRoot { ns = "" }
	if err := r.authorize(ns, group, res, "get", args.Name); err != nil { return mcp.ToolResult{}, err }
...
func compactReasons(items []map[string]any, key string) string {
	...
	out := []string{}
	for _, item := range items { if v, ok := item[key].(string); ok && v != "" && !seen[v] { seen[v] = true; out = append(out, v) } }
